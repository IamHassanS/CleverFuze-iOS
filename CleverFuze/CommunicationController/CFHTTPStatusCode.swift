...
enum CFHTTPStatusCode: Int, Error {
...
extension HTTPURLResponse {
    ...
    var status: CFHTTPStatusCode? {
        return CFHTTPStatusCode(rawValue: statusCode)
    }
    
}

extension CFHTTPStatusCode: Codable{
  
  init(from decoder: Decoder) throws {
    guard let values = try? decoder.singleValueContainer().decode(Int.self) else{
      throw ErrorHandler.nilValueOrNonInteger
    }
    guard let raw = Self(rawValue: values) else{
      throw ErrorHandler.unDefinedType
    }
    self = raw
  }
